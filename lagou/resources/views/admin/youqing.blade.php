...
<body>
	...
	<table>
		...
		@foreach($data as $v)
		<tr>
			<td><input type="text" tid='{{$v->id}}' name="sort" value="{{$v->sort}}" onblur="cuncun(this)" /></td>
			<td><input type="text" tid='{{$v->id}}' name="link_name" value="{{$v->link_name}}" onblur="cuncun(this)" /></td>
			<td><input type="text" tid='{{$v->id}}' name="link_href" value="{{$v->link_href}}" onblur="cuncun(this)" /></td>
			<td><input type="text" tid='{{$v->id}}' name="link_title" value="{{$v->link_title}}" onblur="cuncun(this)" /></td>
		</tr>
		@endforeach
		
	</table>
	<form action="{{url('Admin/link_add')}}" method="post" name="form">
		<table>
...
function cuncun(ts)
{
	...
	var val = $(ts).val();
	$.get("{{url('Admin/link_update')}}",{id:id,name:name,val:val});
}
$('#bt').click(function(){
	str = "<tr>";
